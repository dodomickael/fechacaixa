document.addEventListener('DOMContentLoaded', () => {
    ...
    const totalValueSpan3 = document.getElementById('totalValue3');
    ...
    function updateTotal() {
        ...
        const coinssobrando = parseFloat(document.getElementById('coinssobrando').value) || 0;

        const totalNotas = (notes2 * 2) + (notes5 * 5) + (notes10 * 10) +
                      (notes20 * 20) + (notes50 * 50) + (notes100 * 100) +
                      (notes200 * 200);

        const totalCoins = + (coins005 * 0.05) + (coins010 * 0.10) +
        (coins025 * 0.25) + (coins050 * 0.50) + (coins1 * 1);

        const total = totalCoins + totalNotas + coinssobrando;

        totalValueSpan1.textContent = totalNotas.toFixed(2);
        totalValueSpan2.textContent = totalCoins.toFixed(2);
        totalValueSpan3.textContent = total.toFixed(2);
    }
});
